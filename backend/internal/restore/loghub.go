...
// Seed replaces the in-memory buffer (used when hydrating from SQLite).
func (h *LogHub) Seed(restoreID string, lines []string) {
	if h == nil || restoreID == "" {
		return
	}
	h.mu.Lock()
	...
	copy(cp, lines)
	if len(cp) > maxLogLines {
		cp = cp[len(cp)-maxLogLines:]
	}
	h.buffers[restoreID] = cp
}
